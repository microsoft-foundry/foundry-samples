...
#
# Select sample x toolbox x deploy-mode combinations for the ADO pipeline.
#
# Env inputs:
#   DISCOVERY_MODE          all | changed
#   BASE_REF                git ref to diff against when DISCOVERY_MODE=changed
#   SAMPLE_FILTER           optional comma-separated substring filter; '*' means all
...
emit_entries() {
  ...
  case "$sample_dir" in
    samples/python/hosted-agents/*)
      ...
      # point, keeping the runtime declaration in the sample manifest.
...
if [ "$DISCOVERY_MODE" = "changed" ]; then
  ...
  while IFS= read -r yaml_file; do
    ...
    fixture_dir=$(python3 .github/scripts/hosted_agent_fixture.py fixture-dir --sample-dir "$sample_dir")
