...
validates new sample contracts on public pull requests, including forks subject
to GitHub's workflow approval controls. It uses read-only permissions and no
Azure credentials. Deploying agents and evaluating the evidence described below
are separate cloud E2E responsibilities; the PR policy does not run them.
...
Use Python 3.13, install the policy dependencies, then run both commands from the
repository root:
...
python3 .github/scripts/hosted_agent_test_spec.py validate \
  --spec .azure-pipelines/hosted-agent-tests/<language>/<complete-sample-path>/test-spec.yml
...
as rejecting MCP approval policies for Invocations. The PR policy applies both
checks to new sample contracts. Cloud runners must validate and plan the contracts
they consume before executing their turns.
