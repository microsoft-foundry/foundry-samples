...
USAGE:
    python voice_agent_read_conversation.py
    ...
    3) FOUNDRY_VOICE_CONVERSATION_ID - Optional. The id of a persisted
       ...
       voice_agent_util.py in this folder and
       voice_agent_realtime_text_conversation.py for a full interactive
       version.
...
from voice_agent_util import hold_sample_conversation
...
with (
    ...
    try:
        ...
        for response in conversations.list_responses(agent_name, conversation_id):
            ...
            # The items produced by this specific response. Conversation items are a typed,
            # discriminated union (``RealtimeConversationItem``); ``.type`` is on every item, but
            # ``.id`` is declared per concrete subtype, so read it with getattr.
            for response_item in conversations.list_response_items(agent_name, conversation_id, response.id):
                print(f"      item {response_item.type} id={getattr(response_item, 'id', None)}")
