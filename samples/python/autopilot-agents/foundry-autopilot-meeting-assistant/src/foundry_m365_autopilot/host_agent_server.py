"""Activity Protocol host for the Meeting Assistant Digital Worker sample."""
...
from .agent import _is_meeting_list_command
from .agent_interface import AgentInterface, check_agent_inheritance
from .activity_filter import ActivityDeduplicator, is_agent_authored_activity
...
from .meeting_store import extract_meeting_id
from .token_cache import cache_agentic_token, get_cached_agentic_token
from .upcoming_meetings import build_upcoming_meetings_attachment
...
async def _update_or_send_card(
    ...
    activity_id = str(getattr(context.activity, "reply_to_id", "") or "").strip()
    if activity_id:
        try:
            ...
            logger.exception("Failed to update meeting reply configuration card")
...
class GenericAgentHost:
    def __init__(
        ...
        **agent_kwargs,
        ...
        self._activity_deduplicator = ActivityDeduplicator()
    ...
    def _setup_handlers(self) -> None:
        ...
        async def help_handler(context: TurnContext, _: TurnState) -> None:
            await context.send_activity(
                Activity(
                    type="message",
                    text="Meeting Assistant capabilities",
        ...
        async def on_message(context: TurnContext, _: TurnState) -> None:
            if is_agent_authored_activity(context.activity):
                logger.info("Ignoring an activity authored by the Agent")
                return
            if self._activity_deduplicator.is_duplicate(context.activity):
                logger.warning(
                    "Ignoring duplicate Activity id=%s",
                    getattr(context.activity, "id", "") or "(missing)",
                )
                return
            ...
            if submit_verb == CONFIGURE_RULES_VERB:
                attachment = await self.agent_instance.build_meeting_delegate_configuration_card(
                    str(submit_data.get("meetingId") or ""),
                    self.agent_app.auth,
                    self.auth_handler_name,
                    context,
                    meeting_metadata=submit_data,
                )
                await context.send_activity(
                    Activity(
                        type="message",
                ...
                return
            if submit_verb in {ADD_RULE_VERB, REMOVE_RULE_VERB, SAVE_RULES_VERB}:
                attachment = await self.agent_instance.handle_meeting_delegate_card_action(
                    submit_verb,
                    submit_data,
                    self.agent_app.auth,
                    self.auth_handler_name,
            ...
            if not user_message.strip() or user_message.strip() == "/help":
                return
            if is_submit_message(user_message):
                return

            if extract_meeting_id(context.activity, user_message):
            ...
            if user_message.strip().casefold().startswith("/meeting_delegate"):
                remainder = user_message.strip()[len("/meeting_delegate") :].strip()
                command, _, payload = remainder.partition(" ")
                if command.casefold() == "configure" and not payload.lstrip().startswith("{"):
                    attachment = await self.agent_instance.build_meeting_delegate_configuration_card(
                        payload,
                        self.agent_app.auth,
                        self.auth_handler_name,
            ...
            if _is_meeting_list_command(user_message):
                intent = self.agent_instance.LIST_MEETINGS_INTENT
            else:
            ...
            if intent == self.agent_instance.CAPABILITIES_INTENT:
                ...
                return
            if intent == self.agent_instance.CONFIGURE_DELEGATE_INTENT:
                meeting_id = extract_configuration_meeting_id(user_message)
                attachment = await self.agent_instance.build_meeting_delegate_configuration_card(
                    meeting_id,
                    self.agent_app.auth,
                    self.auth_handler_name,
                ...
                return
            if intent == self.agent_instance.LIST_MEETINGS_INTENT:
                result = await self.agent_instance.list_upcoming_meetings(
                    self.agent_app.auth,
                    self.auth_handler_name,
                    context,
                )
                await context.send_activity(
                    Activity(
                        ...
                        attachments=[build_upcoming_meetings_attachment(result)],
                    )
                )
                return
    ...
    def start_server(
        ...
        async def root(_req: Request) -> Response:
            return Response(text="Foundry Autopilot Meeting Assistant is running.")
