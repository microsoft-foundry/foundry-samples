...
var mailMonitorStorageAccountName = '${take(toLower(replace(accountName, '-', '')), 11)}${uniqueString(subscription().id, resourceGroup().id)}'

resource mailMonitorStorageAccount 'Microsoft.Storage/storageAccounts@2023-05-01' = {
  name: mailMonitorStorageAccountName
  location: location
  tags: tags
  sku: {
    name: 'Standard_LRS'
...
resource mailMonitorBlobService 'Microsoft.Storage/storageAccounts/blobServices@2023-05-01' = {
  parent: mailMonitorStorageAccount
  name: 'default'
}

resource mailMonitorContainer 'Microsoft.Storage/storageAccounts/blobServices/containers@2023-05-01' = {
  parent: mailMonitorBlobService
  name: 'mail-monitor-config'
  properties: {
    publicAccess: 'None'
  }
}
...
output mailMonitorStorageAccountUrl string = mailMonitorStorageAccount.properties.primaryEndpoints.blob

output mailMonitorStorageAccountId string = mailMonitorStorageAccount.id

output mailMonitorStorageContainerName string = mailMonitorContainer.name
