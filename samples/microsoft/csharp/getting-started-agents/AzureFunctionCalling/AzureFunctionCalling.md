...
First, we set up the necessary configuration, initialize the `PersistentAgentsClient`, define the `AzureFunctionToolDefinition` for our Azure Function, and then create the agent. This step includes all necessary `using` directives.
...
Next, we create a new persistent agent thread and add an initial user message to it.
...
Then, we create a run for the agent on the thread and poll its status until it completes or requires action.
